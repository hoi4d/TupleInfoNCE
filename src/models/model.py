...
from src.models.rgb_depth_fusion import SqueezeAndExciteFusionAdd3
...
class ESANetP4(nn.Module):
    def __init__(self,
        ...
        self.fuse_depth_in_rgb_encoder = fuse_depth_in_rgb_encoder
        self.globalmaxpool = nn.AdaptiveMaxPool2d(1, return_indices=False)
        # set activation function
        if activation.lower() == 'relu':
            self.activation = nn.ReLU(inplace=True)
        elif activation.lower() in ['swish', 'silu']:
        ...
        # new encoder
        if encoder_new == 'resnet18':
            self.encoder_new = ResNet18(
                block=encoder_block,
                pretrained_on_imagenet=pretrained_on_imagenet,
                pretrained_dir=pretrained_dir,
                activation=self.activation)
        elif encoder_new == 'resnet34':
            self.encoder_new = ResNet34(
                block=encoder_block,
                pretrained_on_imagenet=pretrained_on_imagenet,
                pretrained_dir=pretrained_dir,
                activation=self.activation)
        elif encoder_new == 'resnet50':
            self.encoder_new = ResNet50(
                pretrained_on_imagenet=pretrained_on_imagenet,
                activation=self.activation)
        else:
            raise NotImplementedError(
                'Only ResNets are supported for '
                'encoder_new. Got {}'.format(encoder_new))


        self.channels_decoder_in = self.encoder_rgb.down_32_channels_out

        if fuse_depth_in_rgb_encoder == 'SE-add':
            self.se_layer0 = SqueezeAndExciteFusionAdd3(
                64, activation=self.activation)
            self.se_layer1 = SqueezeAndExciteFusionAdd3(
                self.encoder_rgb.down_4_channels_out,
                activation=self.activation)
            self.se_layer2 = SqueezeAndExciteFusionAdd3(
                self.encoder_rgb.down_8_channels_out,
                activation=self.activation)
            self.se_layer3 = SqueezeAndExciteFusionAdd3(
                self.encoder_rgb.down_16_channels_out,
                activation=self.activation)
            self.se_layer4 = SqueezeAndExciteFusionAdd3(
                self.encoder_rgb.down_32_channels_out,
                activation=self.activation)
    ...
    def forward(self, rgb, depth, new):
        rgb = self.encoder_rgb.forward_first_conv(rgb)
        depth = self.encoder_depth.forward_first_conv(depth)
        new = self.encoder_new.forward_first_conv(new)

        if self.fuse_depth_in_rgb_encoder == 'add':
            fuse = rgb + depth + new
        else:
            fuse = self.se_layer0(rgb, depth, new)

        rgb = F.max_pool2d(fuse, kernel_size=3, stride=2, padding=1)
        depth = F.max_pool2d(depth, kernel_size=3, stride=2, padding=1)
        new = F.max_pool2d(new, kernel_size=3, stride=2, padding=1)

        # block 1
        rgb = self.encoder_rgb.forward_layer1(rgb)
        depth = self.encoder_depth.forward_layer1(depth)
        new = self.encoder_new.forward_layer1(new)
        if self.fuse_depth_in_rgb_encoder == 'add':
            fuse = rgb + depth +new
        else:
            fuse = self.se_layer1(rgb, depth, new)
        skip1 = self.skip_layer1(fuse)

        # block 2
        rgb = self.encoder_rgb.forward_layer2(fuse)
        depth = self.encoder_depth.forward_layer2(depth)
        new = self.encoder_new.forward_layer2(new)
        if self.fuse_depth_in_rgb_encoder == 'add':
            fuse = rgb + depth + new
        else:
            fuse = self.se_layer2(rgb, depth, new)
        skip2 = self.skip_layer2(fuse)

        # block 3
        rgb = self.encoder_rgb.forward_layer3(fuse)
        depth = self.encoder_depth.forward_layer3(depth)
        new = self.encoder_new.forward_layer3(new)
        if self.fuse_depth_in_rgb_encoder == 'add':
            fuse = rgb + depth + new
        else:
            fuse = self.se_layer3(rgb, depth, new)
        skip3 = self.skip_layer3(fuse)

        # block 4
        rgb = self.encoder_rgb.forward_layer4(fuse)
        depth = self.encoder_depth.forward_layer4(depth)
        new = self.encoder_new.forward_layer4(new)
        if self.fuse_depth_in_rgb_encoder == 'add':
            fuse = rgb + depth + new
        else:
            fuse = self.se_layer4(rgb, depth, new)

        feat = self.globalmaxpool(fuse)
        outk = self.context_module(fuse)
        out = self.decoder(enc_outs=[outk, skip3, skip2, skip1])
...
#             fuse = self.se_layer0(rgb, depth, new)
#
#         new = F.max_pool2d(fuse, kernel_size=3, stride=2, padding=1)
#         depth = F.max_pool2d(depth, kernel_size=3, stride=2, padding=1)
...
#
#         # block 2
#         new = self.encoder_new.forward_layer2(fuse)
#         depth = self.encoder_depth.forward_layer2(depth)
...
#
#         # block 3
#         new = self.encoder_new.forward_layer3(fuse)
#         depth = self.encoder_depth.forward_layer3(depth)
...
#
#         # block 4
#         new = self.encoder_new.forward_layer4(fuse)
#         depth = self.encoder_depth.forward_layer4(depth)
