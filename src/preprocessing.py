...
class Rescale:
    ...
    def __call__(self, sample):
        image, depth, new = sample['image'], sample['depth'], sample['new']

        image = cv2.resize(image, (self.width, self.height),
                           interpolation=cv2.INTER_LINEAR)
        depth = cv2.resize(depth, (self.width, self.height),
                           interpolation=cv2.INTER_NEAREST)
        new = cv2.resize(new, (self.width, self.height),
                           interpolation=cv2.INTER_NEAREST)
        sample['image'] = image
        sample['depth'] = depth
        sample['new'] = new
...
class RandomRescale:
    ...
    def __call__(self, sample):
        image, depth, new, label = sample['image'], sample['depth'], sample['new'], sample['label']
        ...
        depth = cv2.resize(depth, (target_width, target_height),
                           interpolation=cv2.INTER_NEAREST)
        new = cv2.resize(new, (target_width, target_height),
                           interpolation=cv2.INTER_NEAREST)
...
class RandomCrop:
    ...
    def __call__(self, sample):
        ...
        if h <= self.crop_height or w <= self.crop_width:
            ...
            new = new[i:i + self.crop_height, j:j + self.crop_width]
...
class Normalize:
    ...
    def __call__(self, sample):
        ...
        image = torchvision.transforms.Normalize(
            mean=[0.485, 0.416, 0.398], std=[0.288, 0.295, 0.308])(image)     #mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]
        new = torchvision.transforms.Normalize(
            mean=[0.184, 0.430, 0.501], std=[0.162, 0.244, 0.247])(new)
...
class ToTensor:
    def __call__(self, sample):
        ...
        sample['new'] = torch.from_numpy(new).float()
...
class ONE_RandomCrop:
    def __init__(self, crop_height, crop_width, d):
        self.crop_height = crop_height
        self.crop_width = crop_width
        self.rescale = Rescale(self.crop_height, self.crop_width)
        self.d = d
    ...
    def __call__(self, sample):
        ...
        if h <= self.crop_height or w <= self.crop_width:
            ...
            j = 0
            image = image[i:i + self.crop_height, j:j + self.crop_width, :]
            depth = depth[i+self.d:i + self.crop_height+self.d, j+self.d:j + self.d + self.crop_width]
            new = new[i+self.d:i + self.crop_height+self.d, j+self.d:j + self.d + self.crop_width]
            sample['image'] = image
            sample['depth'] = depth
            sample['new'] = new
...
class ONE_RandomHSV:
    ...
    def __call__(self, sample):
        ...
        # s_random = np.random.uniform(min(self.s_range), max(self.s_range))
        # v_random = np.random.uniform(-min(self.v_range), max(self.v_range))
        h_random = np.random.choice(self.h_range)
        s_random = np.random.choice(self.s_range)
        v_random = np.random.choice(self.v_range)
        img_h = np.clip(img_h * h_random, 0, 1)
        img_s = np.clip(img_s * s_random, 0, 1)
        img_v = np.clip(img_v + v_random, 0, 255)
        img_hsv = np.stack([img_h, img_s, img_v], axis=2)
