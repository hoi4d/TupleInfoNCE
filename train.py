...
def train_main():
    ...
    if args.P4 != '':
        print('load weight pretrained by P4')
        model.load_state_dict(torch.load(args.P4))
...
def train_one_epoch(model, train_loader, device, optimizer, loss_function_train,
    ...
    # num_of_pixels = 795 * 640 * 480
    # total_sum = 0
    # for batch in train_loader:
    #     total_sum += batch['new'][:,0,:,:].sum()
    # mean = total_sum / num_of_pixels
    # sum_of_squared_error = 0
    # for batch in train_loader:
    #     sum_of_squared_error += ((batch['new'][:,0,:,:] - mean).pow(2)).sum()
    ...
    for i, sample in enumerate(train_loader):
        # print('i: '+str(i))
        # print('sample: '+str(sample['disturb'].size()))
        ...
        # disturb = sample['disturb'].to(device)
        # disturb = disturb.float()
        ...
        # forward pass
        if modality == 'rgbd':
            pred_scales, _ = model(image, depth,new)
        elif modality == 'rgb':
        ...
        with valid_loader.dataset.filter_camera(camera):
            ...
            for i, sample in enumerate(valid_loader):
                ...
                with torch.no_grad():
                    ...
                    if modality == 'rgbd':
                        prediction, _ = model(image, depth, new)
                    elif modality == 'rgb':
